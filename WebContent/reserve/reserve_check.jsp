...
<%	int page1;
	...
	ArrayList<ReserveDto> rlist = rdao.reserve_check(session,index);
	
	int cnt = rdao.page_check_count(session);
...
<div id="third">
	 ...
	 <table class="my_reserve" align="center">
			...
			<tr>
				<td height="60" colspan="6" align="center" >내가 작성한 예약</td>
			</tr>
			<tr>
				<td>제목</td>
				<td>성별</td>
			...
			<tr>
				<td><a href="reserve_check_content.jsp?id=${rdto.id}">${rdto.title}</a></td>
			...
			<c:if test="${page1 == i}">
			 ...
			 <c:if test="${page1 != i}">
				<a href="reserve_check.jsp?page1=${i}">${i}</a>
			</c:if>
			</c:forEach>
	</table>
	</div>
