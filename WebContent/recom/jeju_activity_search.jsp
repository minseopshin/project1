...
<%   int page1;
   ...
   ArrayList<JejuActivityDto> jlist = jdao.jejuactivity_search(request,index);
   
   int cnt = jdao.jeju_activity_search_cnt(request);
   int page_cnt = cnt/10;
   if(cnt%10 != 0 )   {
      page_cnt++;
   }
...
<body>
      ...
      <c:forEach var="jdto" items="${jlist}">
            <div style="float:left; padding-left:40px; display: inline-block; margin:auto;"><a href="jeju_activity_content.jsp?id=${jdto.id}" target=".parent"><img id="pho" src="${jdto.imgpath}" width="300" height="200"></a><p>
            <span id="title"><a href="jeju_activity_content.jsp?id=${jdto.id}" target=".parent">${jdto.title}</a></span>
            </div>
...
<td >
 <c:forEach var="i" begin="${pagestart}" end="${pageend}">
      ...
      <c:if test="${page1 != i}">
         <span style="color:black;font-size:20px;text-decoration:none;"><a href="jeju_activity_search.jsp?searchmenu=${searchmenu}&search=${search}&page1=${i}" style="text-decoration:none;color:black;">${i}</a></span>
      </c:if>
   </c:forEach>
   </td>
