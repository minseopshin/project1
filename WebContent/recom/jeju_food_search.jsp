...
<%@ page import="project.dao.JejuFoodDao" %>
<%@ page import="project.dto.JejuFoodDto" %>
...
<%   int page1;
   ...
   JejuFoodDao jdao = new JejuFoodDao(); 
   ArrayList<JejuFoodDto> jlist = jdao.jejufood_search(request,index);
   
   int cnt = jdao.jeju_food_search_cnt(request);
   int page_cnt = cnt/10;
   if(cnt%10 != 0 )   {
      page_cnt++;
   }
...
<body>
      ...
      <c:forEach var="jdto" items="${jlist}">
            <div style="float:left; padding-left:40px;  margin:auto;"><a href="jeju_food_content.jsp?id=${jdto.id}" target=".parent"><img id="pho" src="${jdto.imgpath}" width="300" height="200"></a><p>
            <span id="title"><a href="jeju_food_content.jsp?id=${jdto.id}" target=".parent">${jdto.title}</a></span>
            </div>
...
<td >
 <c:forEach var="i" begin="${pagestart}" end="${pageend}">
      ...
      <c:if test="${page1 != i}">
         <span style="color:black;font-size:20px;text-decoration:none;"><a href="jeju_food_search.jsp?searchmenu=${searchmenu}&search=${search}&page1=${i}" style="text-decoration:none;color:black;">${i}</a></span>
      </c:if>
   </c:forEach>
   </td>
