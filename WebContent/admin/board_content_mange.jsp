...
<!-- 모든 문서의 내용 시작 -->
	 ...
	 <table class="my_reserve" align="center">
	  ...
	  <caption style="padding-bottom:30px"> 후기 게시판 관리 </caption>
	 ...
	 <tr align="center">
	 	...
	 	<td> <a href="../board/content.jsp?id=<%=rs.getString("id")%>"> <%=rs.getString("title")%></a> </td>
	 	...
	 	{
	 	%>
	 	<a onclick="bc1()" href="blind_content.jsp?id=<%=rs.getString("id")%>&state=<%=rs.getString("state")%>"> 클릭 </a> 
	 	<%
	 	}
	 	else
	 	{
	 	%>
	 	<a onclick="bc2()" href="blind_content.jsp?id=<%=rs.getString("id")%>&state=<%=rs.getString("state")%>"> 클릭 </a>
	 	<%
	 	}
	 	%>
	 	</td>
	 	<td> <a onclick="dc()" href="delete_content.jsp?id=<%=rs.getString("id")%>"> 클릭 </a> </td>
	 </tr>
	 </tr>
	 <%
	 }
