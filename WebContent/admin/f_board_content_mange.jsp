...
<!-- 모든 문서의 내용 시작 -->
	<div class="my_reserve_wrap" align="center">
	 <table class="my_reserve" align="center">
	  ...
	  <caption style="padding-bottom:30px"> 외국인 게시판 관리 </caption>
	 ...
	 <tr align="center">
	 	...
	 	<td> <a href="../board2/content2.jsp?id=<%=rs.getString("id")%>"> <%=rs.getString("title")%></a> </td>
	 	...
	 	{
	 	%>
	 	<a onclick="bc1()" href="blind_content2.jsp?id=<%=rs.getString("id")%>&state=<%=rs.getString("state")%>"> 클릭 </a>
	 	<%
	 	}
	 	else
	 	{
		%>
		<a onclick="bc2()" href="blind_content2.jsp?id=<%=rs.getString("id")%>&state=<%=rs.getString("state")%>"> 클릭 </a>
		<%	 
	 	}
	 	%>
	 	</td>
	 	<td> <a onclick="dc()" href="delete_content2.jsp?id=<%=rs.getString("id")%>&state=<%=rs.getString("state")%>"> 클릭 </a> </td>
	 </tr>
    ...
    <tr>
      ...
      {
      %>
       <a href="f_board_content_mange.jsp?pager=<%=pstart-1%>"> ◀◀  </a>
      <%
      }
      else
      {
      ...
      <%
        ...
        {
      %>
       <a href="f_board_content_mange.jsp?pager=<%=pager-1%>"> ◀ </a>
      <%
        }
        else
        {
      ...
      String sql2="select count(*) as cnt from board2";
      ...
      if(cnt%10 != 0)
        ...
        for(int i=pstart;i<=pend;i++)
        {
           if(pager==i) 			
        	...
        	str="";
      %>
        <a href="f_board_content_mange.jsp?pager=<%=i%>" <%=str%>> <%=i%> </a>
      <%
        }
      %>
      ...
      {
      %>
       <a href="f_board_content_mange?pager=<%=pend+1%>"> ▶▶  </a>
      <%
      }
      else
      {
